...
/**
 * This class is used for parsing workflow files: 1. DOT graphs; 2. Pegasus dax files; 3. JSON file
 * @author wu
 */
public class WorkflowParser {
	public static boolean exportToDOTFile(Workflow wf, String file){
		StringBuilder sb = new StringBuilder();
		sb.append("digraph {\n" );
		for(Task t : wf.getTaskList()){
			sb.append(t.getName() + " [label=\""+t.getName()+":"+t.getTaskSize()+"\"];\n");
		}
		for(Task t : wf.getTaskList()){
			for(Edge e : t.getOutEdges()){	/* An Edge. */
				String label = " [label="+e.getDataSize()+"]";
				sb.append(t.getName() + " -> " + e.getDestination().getName() + label + ";\n"); 
			}
		}
		sb.append("}\n");
		try{
	...
	// ע�⣬������̱������Parser���У���Ϊparse XMLʱ��Դ��Ŀ���������õġ�
	static void addDummyTasks(List<Task> list) {
